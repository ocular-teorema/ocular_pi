...
{
    CommonPipelineParameters params = (DataDirectoryInstance::instance())->getPipelineParams();

    m_pIntervalTimer = new IntervalTimer(params.processingIntervalSec());
    ...
    m_fileNamePattern = QString::fromStdString(params.archivePath());
    m_fileNamePattern += '/';
    m_fileNamePattern += QString::fromStdString(params.pipelineName());
    m_fileNamePattern += '/';
    m_fileNamePattern += QString::fromStdString(params.pipelineName());
    m_fileNamePattern += "_%time%.mp4";

    m_shortNamePattern += '/';
    m_shortNamePattern += QString::fromStdString(params.pipelineName());
    m_shortNamePattern += '/';
    m_shortNamePattern += QString::fromStdString(params.pipelineName());
...
void StreamRecorder::WriteEventFile(EventDescription event)
{
    ...
    videoPath.sprintf("%s/%s/alertFragments/alert%d.mp4",
                      pDataDirectory->getPipelineParams().archivePath().data(),
                      pDataDirectory->getPipelineParams().pipelineName().data(),
                      event.id);
