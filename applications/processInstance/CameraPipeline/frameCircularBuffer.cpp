...
void FrameCircularBuffer::GetFrame(VideoFrame** pCurrentFrame, double time)
{
    ...
    if (pDataDirectory->getParams().useVirtualDate())
    {
        ...
        int64_t   frameDuration = 1000 / pDataDirectory->getPipelineParams().fps(); // in milliseconds

        newTime.setHMS( pDataDirectory->getParams().dateParams().hour(),
                        pDataDirectory->getParams().dateParams().minute(),
                        0);

        newDate.setDate( pDataDirectory->getParams().dateParams().year(),
...
void FrameCircularBuffer::AddFrame(AVFrame* pNewFrame, double time)
{
    ...
    stats.setValue("Input Queue Length", m_totalWritten - m_totalRead);
