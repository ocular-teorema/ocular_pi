...
RGB24Buffer *IntervalStatistics::toImage()
{
    // Percent of difference area in frame to consider it as a motion
    double          MotionThreshold = (DataDirectoryInstance::instance())->getParams().totalThreshold();
    const int       canvasW = 200;          // Picture width
    const int       canvasH = 200;          // Picture height
    double          max = *std::max_element(perFrameMovements.constBegin(), perFrameMovements.constEnd());

    double          maxScale = MAX(10.0, MIN(40.0, max));   // Level of maximum displayed motion percent

    double dy = (double)canvasH / maxScale;
    double dx = (double)perFrameMovements.size() / (double)canvasW;

    RGB24Buffer *result = new RGB24Buffer(canvasH, canvasW, (max > MotionThreshold) ? RGBColor(250,105,42) : RGBColor(156,255,64));

    for (int i = 0; i < result->w; i++)
    {
        double maxInterval = 0.0;
        for (int j = i * dx; (j <= dx * (i + 1)) && (j < perFrameMovements.size()); j++)
        {
            maxInterval = MAX(maxInterval, perFrameMovements[j]);
        }
        maxInterval *= dy;
        maxInterval = MIN(maxInterval, canvasH - 2);

        result->drawVLine(i, (result->h - 2) - maxInterval, result->h - 2, RGBColor::Yellow());
    }
    return result;
}
    ...
    m_pCurrentFrame(NULL)
{
    ...
    m_pIntervalTimer = new IntervalTimer(pDataDirectory->getPipelineParams().processingIntervalSec());
...
void VideoStatistics::ProcessAnalyzedFrame(VideoFrame *pCurrentFrame, AnalysisResults *results)
{
    ...
    if ((!pDataDirectory->getParams().differenceBasedAnalysis()) && (!pDataDirectory->getParams().motionBasedAnalysis()))
    {
        DEBUG_MESSAGE0("VideoStatistics", "Do not update statistics when analysis is disabled");
        return;
    }
    ...
    stats.resetInterval("Stats accumulation");
...
void StatisticDBInterface::DelayedGet()
{
    ...
    int                         periodDays = pDataDirectory->getPipelineParams().statisticPeriodDays();
    int                         intervalSeconds = pDataDirectory->getPipelineParams().statisticIntervalSec();
...
void StatisticDBInterface::DelayedWrite()
{
    ...
    newRecord.mediaSource   = QString::fromStdString(pDataDirectory->getPipelineParams().inputStreamUrl());
...
QImage StatisticDBInterface::CreateHeatmap(VideoBuffer* pBkgrBuffer, AccumlatorBuffer* pAccBuffer)
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        {
            val1 = pAcc[MIN(accHeight, (int)(j*scale))*accStride + MIN(accWidth, (int)(i*scale))];
            val2 = pBkg[j*stride + i];

            val1  = MIN(val1, 255.0f) / 255.0f;
            val2 /= 255.0f;
