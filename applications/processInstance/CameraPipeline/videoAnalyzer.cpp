...
void VideoAnalyzer::StartAnalyze()
{
    ...
    m_pDenoiseFilter = new DenoiseFilter();
    m_pObjectDetector = new ObjectDetector();

    DataDirectory* pDataDirectory = DataDirectoryInstance::instance();
    CommonPipelineParameters cmnParams = pDataDirectory->getPipelineParams();
    AnalysisParams params = pDataDirectory->getParams();

    m_frameInterval = 1000.0/(double)cmnParams.fps();
...
void VideoAnalyzer::DoAnalyze()
{
    ...
    m_fpsStats.endInterval("Interframe interval");
    pDataDirectory->addStats(m_fpsStats);
    m_fpsStats.startInterval();
