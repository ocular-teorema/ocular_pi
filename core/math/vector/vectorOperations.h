...
public:
    ...
    /** Elementwize min/max **/
    inline ReturnType minimum(const RealType &other) const
    {
        int length = _size() < other._size() ? _size() : other._size();
        ReturnType result = _createVector(length);

        for (int i = 0; i < length; i++)
        {
            result._at(i) = _at(i) < other._at(i) ? _at(i) : other._at(i);
        }
        return result;
    }


    inline ReturnType maximum(const RealType &other) const
    {
        int length = _size() < other._size() ? _size() : other._size();
        ReturnType result = _createVector(length);
