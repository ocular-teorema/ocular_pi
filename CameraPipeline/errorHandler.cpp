...
ErrorCode ErrorHandler::RenameLogFile(QString newName)
{
    m_logFile.close();
    if (!m_logFile.rename(newName))
    {
        ErrorMessage(ERR_TYPE_ERROR, "ErrorHandler", "Unable to open rename log file");
        return CAMERA_PIPELINE_ERROR;
    }
    return CAMERA_PIPELINE_OK;
}
