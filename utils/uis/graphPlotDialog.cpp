...
void GraphPlotDialog::childRepaint(QPaintEvent * /*event*/, QWidget *who)
{
    ...
    for (int visId = 0; visId < mUi.visibleGraph->rowCount(); visId++) {
        ...
        if (item == NULL)
        {
            qDebug() << "GraphPlotDialog::childRepaint():internal error at id:"  << visId;
            continue;
        }
        unsigned graphId = item->data(Qt::UserRole).toUInt();
        GraphHistory &graph = mData.mData[graphId];
        QPen pen;
        pen.setColor(item->foreground().color());
        if (mData.mData[graphId].isSelected && mGraphPlotParameters->selectGraph()) {
    ...
    if (mUi.pauseButton->isChecked()) {
        if (mGraphPlotParameters->fixGridValue()) {
            ...
            if (mGraphShowPoint >= (int)mData.mData[0].size()) {
                mGraphShowPoint = (int)(mData.mData[0].size() - mGraphShowPoint - 1);
            }
        }
        else {
            if (mGraphShowPoint < 0) {
                mMouseClickedValue = fround(double(mData.mData[0].size() - mGraphShowPoint - 1) * gainX);
            }
            else {
...
void GraphPlotDialog::setGraphVisibility(int currentVisibility)
{
    ...
    unsigned graphId = item->data(Qt::UserRole).toUInt();
    mData.mData[graphId].isSelected = false;
...
void GraphPlotDialog::selectGraph(QTableWidgetItem* current,QTableWidgetItem* prev)
{
    ...
    if (current != NULL) {
        ...
        mData.mData[graphId].isSelected = true;
    }
}
...
void GraphPlotDialog::saveGraphName(QTableWidgetItem*item)
{
    ...
    if (item != NULL && graphList->column(item) == 0) {
        unsigned graphId = item->data(Qt::UserRole).toUInt();
        mData.mData[graphId].name = item->text().toStdString();
...
//    qDebug("GraphPlotDialog::addGraphPoint(%d, %lf, %s): called", graphId, value, isValid ? "valid" : "invalid");
    ...
    mData.addGraphPoint(graphId, value, isValid);
...
void GraphPlotDialog::replaceData(const GraphData &newData)
{
    /* Clean current uis*/
    mUi.visibleGraph->setRowCount(0);
    mUi.invisibleGraph->setRowCount(0);

    /* Replace data*/
    mData = newData;

    for (uint i = 0; i < mData.mData.size(); i++)
    {
        addNewGraphToUI(i);
    }
    update();
...
{
    QTableWidgetItem* graphItem = new QTableWidgetItem(QString::fromStdString(mData.mData[graphId].name));
...
{
    unsigned graphId = mData.getGraphId(name.toStdString());
...
{
    return QString::fromStdString(mData.getGraphName(id));
}
